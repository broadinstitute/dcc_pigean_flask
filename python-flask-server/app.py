...
@app.route("/trapi_analysis", methods=["POST"])
def post_translator_trapi_analysis():
    # initialize 
    map_result = {}
    list_input_genes = []
    sql_conn_query = sql_utils.db_sqlite_get_connection(db_path=db_file)
    ...
    # get the input genes from the trapi json
    data = request.get_json()
    if data:
        list_input_genes = []
        map_genes = tran_utils.get_genes_from_trapi(json_trapi_result=data)
        list_input_genes = list(map_genes.values())

    logger.info("got trapi result request: {} with gene inputs: {}".format(request.method, list_input_genes))
    str_message = "got trapi result gene inputs of size: {}".format(len(list_input_genes))
    logger.info(str_message)
    list_logs.append(str_message)

    # get the p_value
    p_value_cutoff = process_numeric_value(json_request=data, name='p_value', cutoff_default=P_VALUE_CUTOFF)
    str_message = "got trapi result p_value filter: {}".format(p_value_cutoff)
    logger.info(str_message)
    list_logs.append(str_message)

    # get the max gene sets
    max_number_gene_sets = process_numeric_value(json_request=data, name='max_number_gene_sets', cutoff_default=MAX_NUMBER_GENE_SETS_FOR_COMPUTATION, is_float=False)
    str_message = "using trapi result max number of genes: {}".format(max_number_gene_sets)
    logger.info(str_message)
    list_logs.append(str_message)

    # get the gene set family name
    gene_set_family_key = process_string_value(json_request=data, name=dutils.KEY_REST_GENE_SET, default=dutils.KEY_DEFAULT_GENE_SET_FAMILY)
    str_message = "using trapi result input gene set family key: {}".format(gene_set_family_key)
    logger.info(str_message)
    list_logs.append(str_message)
    ...
    if not gene_set_family_object:
        ...
        # map_result['data'] = map_factors
        # list_factor = None
        # list_factor_gene_sets = None
        # list_factor_genes = None
        # list_gene_set_p_values = None
        # logs_process = []
        ...
        # add time
        str_message = "post /trapi_analysis total elapsed time is: {}s".format(end-start)
        logs_process.append(str_message)
        logs_process.append("code version is: {}".format(dutils.get_code_version()))
